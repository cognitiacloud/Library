...
`build-packets/cognitia/receipts.ts`, linked to
`build-packets/cognitia/agent-economy.ts` (Module 15) and
`build-packets/schemas/core.ts`. Sibling docs this spec depends on:
...
`DEALEROS_TMS_TRAFFIC_DESK_SPEC_V1.md` (traffic/SLA receipts), and
`DEALEROS_FEATURE_MAP_AND_MODULE_ARCHITECTURE_V1.md` (module map).
...
1. **Dealers have been burned by vendor claims.** Independent dealers like
   ...
   follow-up and delivered dashboards nobody could audit
   (`DEALEROS_COMPETITOR_RESEARCH_MATRIX_V1.md`). Our counter-position is
   not a louder claim — it is a *paper trail*.
2. **The product metaphor is the spec:** "a dealership sales desk with a
   paper trail — not a magic chatbot" (CONTEXT_PACK Section 5). Every
   important action — human, agent, system, or connector — produces one
   receipt; none can be manufactured later without breaking the chain.
3. **Receipts are the trust product, not a logging feature.** The dealer's
   monthly proof report (Section 10), Demandara reporting (Module 10,
   `GET /api/demandara/proof-report`), agent work-credit candidates
   (Module 15), and compliance audit trails (Module 14) read the same ledger.
4. **Receipts are the AI-governance enforcement point.** Emitter invariants
   ...
   external side effect without a named human approver, or any side effect
   on a denied gate — governance lives in the write path, not a policy PDF.
5. **Internal honesty rule:** receipts prove *what the system recorded, in
   ...
   no public claim may be made from them until validated (CONTEXT_PACK §7).
...
snake_case on purpose (wire/ledger format, serialized to Cognitia as-is);
...
Emitter split: callers supply `ProofReceiptInput` (everything except
...
"Typical gate result" is the default; tenant policy may rule differently.
"Ext" = typical `external_side_effect`.
...
bug; a receipt with no reachable domain record is a bug.
...
1. **Payload hash.** `computePayloadHash()` = sha256 hex of the
   *canonicalized* receipt body — all fields except `payload_hash` and
   `prev_receipt_hash`. Canonicalization recursively sorts object keys and
   drops `undefined` fields, so the same logical receipt always hashes
   identically.
2. **Link hash.** `computeReceiptHash()` = sha256 of
   `payload_hash + '|' + prev_receipt_hash`. Each new receipt stores its
   predecessor's link hash in `prev_receipt_hash`; because the link hash
   covers the predecessor's own `prev_receipt_hash`, it binds the body **and
   the position** of every earlier receipt.
3. **GENESIS anchor.** The first receipt in a tenant ledger stores the
   literal string `'GENESIS'` (`GENESIS_HASH`) — a per-tenant, plain-text
   start-of-chain marker, nothing more.
4. **Verification.** `verifyChain()` walks index 0 → end, checking per entry
   (a) stored `payload_hash` equals a fresh `computePayloadHash()` — failure
   `payload_hash mismatch (receipt body altered)` — and (b) stored
   `prev_receipt_hash` equals `computeReceiptHash(previous)` (or `GENESIS` at
   index 0) — failure `prev_receipt_hash mismatch (chain link broken)`.
   Returns `{ valid, failedAtIndex?, reason? }` to pinpoint the first bad entry.

### 4.2 What tampering detection proves

- Editing any stored receipt field changes its payload hash → detected at
  that index. Deleting, inserting, or reordering receipts breaks the next
  entry's `prev_receipt_hash` → detected at the first affected link.
- An attacker who edits receipt *i* must recompute hashes for *every* later
  receipt — tampering is forced to be wholesale, not surgical.
...
1. **Tamper-evident, not tamper-proof.** A party with full write access can
   ...
   outside the writer's control (periodic export of the latest link hash) —
   a Cognitia-central concern (Section 12), not in the in-process scaffold.
2. **No truth guarantee.** The chain proves internal consistency and
   ordering, not that the described action happened or happened correctly —
   garbage receipted honestly is still garbage. Truth checks come from
   evidence review (`data_refs`, `WORK_EVENT_DEFINITIONS`) and disputes.
...
1. **Templates only.** `claim_safe_summary` comes exclusively from
   `buildClaimSafeSummary()` / `SUMMARY_TEMPLATES` — one fixed template per
   event type, taking only receipt fields (redacted customer ref plus
   optional lead/vehicle/deal/campaign/connector tags). No free text, no
   model-written or caller-supplied summaries (the emitter owns the field).
2. **Style contract:** factual, past tense, one sentence, no superlatives,
   no guarantees, no raw PII. Governance-relevant templates state the
   safeguard in the sentence itself: `ai_reply_drafted` → "…held for human
   approval and was not sent."; `connector_sync_failed` → "…no external
   writes were confirmed for this run."
3. **Banned vocabulary (lint list for any new template, enforced by a
   template test in the dedicated repo):** guarantee(d), best, #1, production
   ready, certified, compliant, always/never/instantly (as performance
   claims), revolutionary, proven (as an outcome claim), closed the deal /
   made the sale (agents draft; humans sell), any token/crypto term. New
   templates are reviewed against CONTEXT_PACK Section 7 before merge.
4. **PII redaction.** Every customer mention is `redactCustomerRef()`:
   `'cust_'` + first 10 hex chars of sha256(customerId) — deterministic per
   customer (receipts for one person correlate), one-way (no raw id or name
   recoverable). Events with no customer use `NO_CUSTOMER_REF`
   (`'cust_none'`); the emitter hard-rejects refs not starting `cust_`.
   Names, phones, emails, and message bodies never enter a receipt — they
   live behind `data_refs`.
5. **Why this matters:** the summary is the only receipt text a dealer,
   auditor, or monthly report ever displays; if the template layer is
...
1. **Shape.** `PolicyGateResult = { gateKey, result, reason }`: the specific
   gate (e.g. `outbound_comms.consent_check`) plus a human-readable reason.
   A verdict without a named gate and reason is not accepted.
2. **Where gates sit.** Gates run *before* the action executes; the verdict
   is recorded *in* the receipt. Flow order: action proposed → gate(s) →
   (a) `allow`: execute, emit receipt; (b) `deny`: do NOT execute, emit a
   receipt recording the denial (`action_requested` ≠ `action_taken`);
   (c) `needs_human_approval`: hold, emit `human_approval_requested`, create
   an approval `WorkTask`, proceed only after `human_approval_granted`.
3. **Emitter enforcement (`assertInvariants`):** a receipt with
   `external_side_effect: true` on a `deny` verdict is rejected — the ledger
   cannot record "we did it anyway"; agent-caused external side effects
   without a `human_approver_id` are rejected.
4. **Default gate set (V1; tenant-configurable via `TenantSettings.approvals.humanApprovalRequiredFor`):**
...
1. **The pair.** `human_approval_requested` and `human_approval_granted` are
   ...
   repeats the same `action_requested`. A grant is valid only if its
   `action_requested` matches its request receipt, linked via `data_refs`
   (request receipt id + approval task ref).
2. **Approval is not execution.** The action that follows (e.g.
   `followup_sent`) is a third receipt with `external_side_effect: true`, the
   same `human_approver_id`, and `data_refs` back to the pair — "who approved
   this send?" is one field plus a two-hop citation, all inside the ledger.
3. **Denials are receipts too.** A rejection is a `HumanReviewEvent` with
   `decision: 'rejected'` (agent-economy.ts) plus a `crm_updated` receipt;
   the action is never re-attempted without a new request. Deliberately no
   `human_approval_denied` event exists — the review event plus the absence
   of a grant is the evidence; the vocabulary stays at exactly 22.
4. **Scale note.** Approval fatigue is accepted for tenant-zero; any
   relaxation (e.g. auto-approving templated follow-ups) is a policy-gate
   change requiring owner sign-off, and remains fully receipted.
...
1. A short instruction for undoing or compensating the action, written at
   emit time; convention `<event>: <compensation>`, e.g.
   `crm_updated: restore prior snapshot ref`.
2. Three classes; the string must be honest about which applies:
   - **Reversible in-system** (crm_updated, vehicle_matched): restore the
     prior snapshot referenced in `data_refs`.
   - **Compensable, not reversible** (followup_sent, review_requested,
     post_listing): a sent message cannot be unsent; the path names the
     compensation (approved correction, channel suppression, delist), itself
     receipted.
   - **Non-reversible record-of-fact** (lead_received, test_drive_completed,
     connector_sync_failed): `none: record-of-fact; correct via superseding
     receipt` — the ledger is append-only; corrections are new receipts.
3. Rollback is never automatic in V1: the path instructs a human (or a
   supervised job), and executing it emits new receipts.

### 8.2 dispute_path

1. A URI-style pointer into the dispute flow,
   `cognitia://disputes/new?receipt=<id>` — every record is contestable by
   construction.
2. Opening a dispute creates a `DisputeEvent` (agent-economy.ts): a named
   `UserId`, a reason, and `open → resolved_upheld | resolved_overturned`.
   Per-work-event dispute guidance (who reviews, what evidence decides) lives
   in `WORK_EVENT_DEFINITIONS` (`cognitia://disputes/new?work_event=…`).
3. On resolved-overturned: the linked `WorkCreditCandidate` moves to
   `rejected`/`disputed`, a negative `ReputationEvent` may be recorded
   against the passport, and a superseding correction receipt is emitted.
   The original receipt is never deleted or edited.

---

## 9. `consent_basis` propagation from `ConsentRecord`

1. **Single vocabulary.** `ConsentBasis = ConsentRecord['basis']` — derived
   from core.ts, so the CASL-aware values (`express`, `implied_ebr`,
   `implied_inquiry`, `none`) cannot drift between CRM and ledger.
2. **Propagation rule.** At action time, the flow resolves the governing
   `ConsentRecord` for the customer + channel and copies its `basis` into
   the receipt — the basis *as of the action*, so a later revocation does
   not retroactively falsify history.
3. **Expiry.** Implied consent expires under CASL (`ConsentRecord.expiresAt`);
   ...
   no consent → deny. The denial receipt still records the expired basis plus
   the gate reason — exactly the evidence an auditor wants.
4. **`none` is legitimate** for events with no communications nexus
   (connector syncs, internal scoring, inventory matches) and inbound
   record-of-fact events. `none` on an outbound `external_side_effect: true`
   receipt is a red-flag ledger query (Section 10) and should be impossible
   if the consent gate is wired correctly — acceptance test material.
5. **Closing the loop:** `consent_captured` receipts write their `receipt_id`
   back onto `ConsentRecord.receiptId` — the consent record and its proof
   cite each other.

---

## 10. Tenant proof ledger UX sketch (dealer-facing; design only, screens built in the dedicated repo)

1. **Receipts browser** (roles: `owner`, `general_manager`, `auditor` —
   read-only by definition in core.ts):
   - Filters: date range, event_type (22-value picker), actor_type, agent
     passport, rooftop, side-effect-only, gate result, consent basis,
     lead/deal/vehicle id.
   - List row: timestamp · event_type badge · claim_safe_summary · actor chip
     · gate verdict chip (green allow / red deny / amber approval) ·
     side-effect icon.
   - Detail drawer: all fields; `data_refs` as role-checked links; approval
     pair navigation; "Open dispute" (dispute_path) and "View rollback
     instructions" (rollback_path) buttons.
   - **Chain status banner:** latest `verifyChain()` result — "Ledger
     verified through receipt N at <time>" or, on failure, `failedAtIndex` +
     reason pinned red. Verification runs on schedule and before every
     export.
2. **Monthly proof report** (per dealer, generated from receipts only; heir
   of the internal "used-car dealer proof report template", CONTEXT_PACK
   Section 5; served via `GET /api/demandara/proof-report`, Module 10):
   - Counts per event type; drafts → approved → sent funnel
     (`ai_reply_drafted` → `human_approval_granted` → `followup_sent`).
   - External side effects listed individually with named approvers.
   - Denied-gate log (what the system refused to do, and why).
   - Consent posture: sends by basis; zero-tolerance check for `none`-basis
     outbound. Disputes opened/resolved; period chain-verification statement.
   - Fixed footer: "Internal record of system activity. Not an outcome
     guarantee. Not audited or certified by any third party."
3. Platform-side access (`platform_admin`) is itself receipted — every
   Cognitia-operator read of a tenant ledger emits an audit record per
   Module 1's audit-log requirement.

---

## 11. Retention and export

1. **Retention is tenant policy:**
   `TenantSettings.retention.receiptRetentionMonths` (core.ts). Proposed
   tenant-zero default: **84 months**, deliberately longer than
   `customerDataMonths` — receipts must outlive what they attest to.
   NEEDS_EXTERNAL_RESEARCH: record-keeping durations advisable under
   PIPEDA / BC PIPA / BC Motor Dealer Act rules are not established by our
   research files. SKIPPED_WITH_REASON: a legally-grounded retention
   schedule requires legal counsel; 84 months is a placeholder.
2. **Right-to-delete interaction.** Receipts contain no raw PII (Section 5),
   so deleting/anonymizing a customer (`Customer.privacy`) never edits
   receipts — the chain survives intact. `data_refs` to purged evidence
   resolve to tombstones ("evidence purged per retention policy at <date>"),
   itself a receipted act. The deterministic `cust_` hash keeps per-customer
   audit trails after deletion; whether it needs a per-tenant salt is
   flagged for the same legal review.
3. **Expiry ≠ silent deletion.** Lapsed receipts move to tenant-controlled
   cold storage; the ledger records a retention-export receipt and the chain
   head continues.
4. **Export format:** JSONL, one canonical-JSON receipt per line in chain
   order, plus a manifest (tenant, range, count, final link hash,
   `verifyChain()` result at export time) — independently re-verifiable with
   ~30 lines of code, no DealerOS software. Surfaces: ledger-browser download
   (owner/auditor) and Module 10 proof endpoints. SKIPPED_WITH_REASON:
   signed/notarized exports need key infrastructure in the dedicated repo.
...
correct semantics, no durability. Intended evolution, in order:
...
2. External chain-head anchoring (Section 4.3 limit #1): periodic checkpoint
   of the latest link hash outside the writer's control, plus scheduled
   re-verification and alerting.
...
4. Cross-product ledger reads: Demandara consuming DealerOS receipts for
   attribution without duplicating them.
5. Central dispute/approval consoles and reputation aggregation over agent
   passports (Module 15 consumers).
...
dedicated infrastructure and the dedicated code repo (recommended:
`cognitiacloud/dealeros`, CONTEXT_PACK Section 9); only the in-process
emitter contract is specified in this Library repo.
...
2. `verifyChain()` returns `valid: true` on an untampered ledger; mutating
   any stored field, or removing/reordering any receipt, yields
   `valid: false` with the correct `failedAtIndex` and reason.
3. Emitter rejects: raw customer refs (not `cust_`-prefixed); side effects on
   `deny`; agent-caused side effects without `human_approver_id`.
4. Every `claim_safe_summary` is template-generated; a lint test proves no
   banned vocabulary (Section 5.3) and no name/phone/email in any receipt.
5. Every `followup_sent` / `review_requested` fixture receipt traces through
   `data_refs` to a matching approval pair with identical `action_requested`.
6. Zero receipts exist with `external_side_effect: true` and
   `consent_basis: 'none'` on a communications event.
7. Export produces chain-ordered JSONL whose manifest verification passes
   on re-import with an independent verifier script.

---

## Boundaries honored

- INTERNAL — DESIGN ONLY. Nothing here is production ready; no public claims
  are authorized from this document or from any receipt content.
- No secrets, no real API keys, no live CRM/DMS writes, no production deploy
  ...
  connectors default to mock; live mode is always gated by human approval
  plus a proof receipt.
- No real customer PII: receipts carry only one-way redacted refs; fixture
  data is fake/reserved/local-only; no outreach is performed or implied.
- No fake proof: receipts describe recorded system activity only; the spec
  documents what the hash chain does NOT prove; the monthly report carries a
  no-guarantee footer.
- No crypto/token/blockchain language or implementation: a single-writer,
  per-tenant, hash-chained append-only log — tamper-evident, not a
  blockchain; the agent-economy linkage stays internal, non-monetary.
- No unsupported compliance claims: CASL/PIPEDA/PIPA handling is
  designed-for, not certified; retention defaults await legal counsel.
- Blocked work is marked inline: receipt signing, legally-grounded retention,
  notarized exports, and the Cognitia central ledger service carry
  SKIPPED_WITH_REASON markers rather than invented designs.
